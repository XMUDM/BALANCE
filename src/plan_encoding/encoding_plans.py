...
def encode_plan_job(plan, parameters):
    plan_encoding_dfs = []
    for node in plan:
        operator_vec, extra_info_vec, condition_vec, has_condition = encode_node_job(node, parameters)
        for i, c_vec in enumerate(condition_vec):
            total_vec = np.concatenate((operator_vec, extra_info_vec, c_vec, np.array([has_condition])), axis=0)
            aa = parameters.physic_op_total_num + parameters.column_total_num + parameters.condition_op_dim + 1
            plan_encoding_dfs.append(np.pad(total_vec, (0,456-aa), 'constant'))
