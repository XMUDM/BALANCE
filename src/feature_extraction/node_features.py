...
def extract_info_from_node(node, alias2table):
    ...
    if node['Node Type'] == 'Materialize':
        ...
        return Hash(), None
    elif node['Node Type'] == 'Sort' or node['Node Type'] == 'Incremental Sort':
        valid_keys = []
        for key in node['Sort Key']:
            if re.match(r'^[a-z][a-zA-Z0-9_]*\.[a-z][a-zA-Z0-9_]*$', key) is not None:
                valid_keys.append(key)
        keys = [change_alias2table(key, alias2table) for key in valid_keys]
        return Sort(keys), None
    elif node['Node Type'] == 'Merge Append':
        keys = [change_alias2table(key, alias2table) for key in node['Sort Key']]
        return MergeAppend(keys), None
        ...
        if 'Filter' in node and node['Parent Relationship'] != 'Inner':
            ...
            condition_seq_filter = []
        condition_seq_index, relation_name, index_name = [], None, None
        return Scan('CTE Scan', condition_seq_filter, condition_seq_index, relation_name, index_name), None
    else:
