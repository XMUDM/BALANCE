...
from stable_baselines.common.tf_layers import conv, linear, conv_to_fc, lstm ,linear_cl
...
def mlp_extractor_my(flat_observations, net_arch, act_fun, loading = False):
    """
    Constructs an MLP that receives observations as an input and outputs a latent representation for the policy and
    a value network. The ``net_arch`` parameter allows to specify the amount and size of the hidden layers and how many
    of them are shared between the policy network and the value network. It is assumed to be a list with the following
    ...
    wide = 19*50
    if(flag_boo):
        cl_in =  tf.concat([latent[:,-temps-wide:-temps], latent[:,-4-size:-4]],1)
        cl_in = linear_cl(cl_in, "cl_fc{}".format(0), 256, loading = loading,init_scale=np.sqrt(2)  ,trainable=False)
        latent = tf.concat([latent[:,:-temps-wide], cl_in[:,:],    latent[:,-temps:]],1)
    ######################################
    # Iterate through the shared layers and build the shared parts of the network
    for idx, layer in enumerate(net_arch):
        if isinstance(layer, int):  # Check that this is a shared layer
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution) or isinstance(self.proba_distribution, MultiCategoricalProbabilityDistribution):
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, loading=False,feature_extraction="cnn", **kwargs):
...
class FeedForwardPolicy_my(ActorCriticPolicy):
    """
    Policy object that implements actor critic, using a feed forward neural network.
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn,loading = False,  feature_extraction="cnn", **kwargs):
        super(FeedForwardPolicy_my, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
                                                scale=False)
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                ...
                ########################### my version
                pi_latent, vf_latent = mlp_extractor_my(tf.layers.flatten(self.processed_obs), net_arch, act_fun,loading=loading)
...
class CnnNewCostPolicy(FeedForwardPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(CnnNewCostPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse, cnn_extractor=new_cost_cnn,
                                        feature_extraction="cnn", **_kwargs)
...
class MlpPolicy_my(FeedForwardPolicy_my):
    """
    Policy object that implements actor critic, using a MLP (2 layers of 64)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(MlpPolicy_my, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                        feature_extraction="mlp", **_kwargs)
