...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None,acc=None):
    ...
    def setup_model(self,loading = False):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.mysoftmax = tf.nn.softmax(asl)
                ...
                ac_softmax_p = exp_logits / tf.reduce_sum(exp_logits, axis=-1, keepdims=True)
                ac_softmax_p_m = tf.multiply(ac_softmax_p, one_hot_actions)
                self.ac_softmax_p_m = tf.reduce_sum(tf.multiply(ac_softmax_p_m, one_hot_actions), axis=-1)
                ################################
# def act_prob(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, action_masks, update,
...
#                   self.old_neglog_pac_ph: neglogpacs, self.old_vpred_ph: values}
    def act_prob(self,  obs,  masks, actions, action_masks
        ...
        #             [self.mysoftmax,self.ac_softmax_p_m],
        #             td_map)
        summary = self.sess.run(
                    [self.mysoftmax],
                    td_map)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                # Unpack
                obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, action_masks,time_test ,count_test = rollout
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        return mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward, mb_action_masks , sum_time_test,sum_count_test
